...
function node(onFinished) {
  ...
  */
  node.prototype.include = (filePath, options)=>{
    ...
    var path = this.mapPath(filePath);
    ...
    if (Path.extname(filePath) !== config.scriptExtension) {
      script = cgiParser.script(this.__scripts.length, path, content.toString());
    }

    // Otherwise just create a new script object
    else {
      script = {
        id: this.__scripts.length,
        path: path,
        script: null,
        code: content,
        content: []
    ...
    vm.runInNewContext(script.code, this, script.path);
  };
  /*
  ...
  */
  node.prototype.info = ()=>{
    ...
    this.response.write('<style>.Logo{ text-align: left; font-size: 36px !important; } .NodeASPTable{ font-family: arial; font-size: 12px; margin: auto; border-collapse: collapse; width: 600px} .NodeASPTable TH{ background-color: #303030; color: white; font-size: 14px; padding: 10px} .NodeASPTable TD{ padding: 5px; } .NodeASPTable TR TD:nth-child(1){ background: #d9ebb3; }</style>');
    ...
    drawObject('Server Variables', this.request.server);
    drawObject('HTTP Request Headers', this.request.headers);
    drawObject('HTTP Request Cookies', this.request.cookies);
    drawObject('Session', session);
    drawObject('Session Cookies', this.session.cookies);
    drawObject('Session Data', this.session.data);
    drawObject('URL Query String', this.request.query);
    drawObject('Post Form', this.request.post.form);
    drawObject('Post Files', this.request.post.files);
    drawObject('Post Parts', this.request.post.parts);
  ...
  */
  node.prototype.script = (code)=>{
    return this.response.write(`<script>${code}</script>`);
  };
  ...
  node.prototype.log = (data) => {
    var timeStamp = this.timeNow.toTimeString();
    var json = {
      time: timeStamp,
      data: data
    };
...
*/
var buildError = (msg, error, stack)=>{
